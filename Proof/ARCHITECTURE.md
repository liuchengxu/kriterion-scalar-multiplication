...
* **What is left, in order.**
  1. Per lazy output: one `PMF.bind_comm` puts `key` outside `familyLaw`; then on the curve
     ...
     `advantage` bound.
  2. **The one remaining tool gap: averaging that per-output bound.** The outer samples
     `u`, `tape`, `carrier`, `raw` are `Fintype`, so `advantage_bind_le_of_le` peels them
     with the **uniform** bound `3 q₁ / 2 ^ 128`; but the lazy-run *output* type contains a
     `List Query` and an adversary state, so it is **not** a `Fintype` and
     `advantage_bind_le_of_le` (which is `bind_apply_sub_le_of_le`, stated with
     `[Fintype Sample]`) does not apply at that level. Either
     (a) prove the `tsum` version -- `advantage (law.bind first) (law.bind second) ≤ bound`
     from a pointwise bound, with no finiteness assumption (`ENNReal.tsum_toReal_eq`,
     `Summable.of_nonneg_of_le` against `∑' s, (law s).toReal = 1`, `tsum_sub`,
     `abs_tsum_le_tsum_abs`, `tsum_le_tsum`) -- **recommended, ~80 lines, reusable**; or
     (b) fold the lazy output into the `advantage_bind_le_bad` sample as
     `(lazy output, fibers, hash, key)`, which then needs the off-curve branch to carry a
     dummy hash sample (`PMF.bind_const`) so that both branches share one sample shape.
  3. `advantage_bind_le_of_le` for `u`, `tape`, `carrier`, `raw`, then
     `steeringCharge_le_steeringStep`, then
