...
peel, and the final `exact`. It also had to **split `Proof/Privacy.lean`**: the chain could
not be imported into it without an import cycle, so the hybrid game and the shared core moved
verbatim into the new `Proof/Hybrid.lean`. No constant moved and no statement changed. See
"Corrections from slice 3r".

Slice 3q built the **structural tool** the slice-3p route needs -- the lazy-sampling
theorem for the **whole permutation family** (`Proof/Family.lean`) -- and, on top of it,
the hop's law equality and its charge in concrete game terms (`Proof/SteeringHop.lean`).
The charge is machine-checked at `3 q₁ / 2 ^ 128` and machine-checked to fit
`steeringStep`. What is still open is the **game reshaping**: putting both games into the
one sample order the charge needs. Read "Corrections from slice 3q" before touching
`Proof/SteeringHop.lean`; it contains one hard ordering constraint that the slice-3p
plumbing plan did not see. `hybridGame_close_to_idealGame` still carries the single
...
**Corrections from slice 3o -- the transfer is DONE; only the bad mass is left.**

* **Slice 3n's order (charge first, transfer second) is confirmed, and the transfer is
  even cheaper than 3n predicted: `twoStageGame_congr_support` is NOT needed for it, and
  neither is `steeredReleasedStageTwo_offCurve`.** Put the curve split in the *hidden
  part* rather than in the second stage. `shiftedOutputs scalar bridge carrier input
  outputs` is `shiftSteering (c/s - u) outputs` on the curve and `outputs` off it; the
  **shifted reference round** (`shiftedReferenceRound`, `Proof/Shifted.lean`) is the
  reference round of `u` whose second stage receives that hidden part. Both it and
  `referenceRound (hybridBridge scalar)` then run the *same* second stage
  (`releasedStageTwo`), so the plain `twoStageGame_congr` closes the transfer:
  `map_releasedPair_onCurve` on the curve (`u + (c/s - u) = c/s`) and
  `map_releasedPair_offCurve` off it. Machine-checked as
  **`shiftedReferenceGame_eq_referenceGame`** -- an *equality*, for **every** bridge key
  `u`, hence `bind_shiftedReferenceGame_eq_referenceGame` by `PMF.bind_const`.
  `steeredReleasedStageTwo_offCurve` is still needed, but in the **charge**, where it
  makes the off-curve half free.
* **The residual obligation is now the charge and nothing else.**
  `workPerAdvantage_of_steeringCharge` (`Proof/Shifted.lean`) is
  `workPerAdvantage_of_steering` with the transfer discharged: it takes
  `advantage ((uniform NonZeroBase).bind fun u => shiftedReferenceGame (fun _ => u.value)
  scalar ...) ((uniform NonZeroBase).bind fun u => steeredReferenceGame (fun _ => u.value)
  scalar ...) <= steeringStep` and returns the obligation's goal. Both games now have the
  same sample, the same first stage and the same released table.
* **The two second stages are the same program on two views, under literally the same two
  fiber samples.** This is the content of `Proof/Charge.lean` and it is all equational:
  * `selectedSimulatedStageTwo_double` -- on the curve
    (`steeringTarget_onCurve`: the simulator's target *is* `hybridBridge scalar carrier`)
    and on a log against which the steering's own requests are fresh, the steered second
    stage is one extra `uniformHashFiber wanted` sample followed by the hybrid second
    stage on the **doubly programmed** view `doubleSteeredOracle`. `wanted` is
    `outputs x7 0 + (c/s - u)` (`steeringWanted_programSelected`), so it is exactly the
    steering gate's shifted output.
  * `releasedStageTwo_shifted` -- the shifted round's *single* fiber sample at the shifted
    outputs splits, exactly, into the honest family plus the steering gate's resample
    (`uniformHashFibers_setSteering` read right-to-left), followed by the hybrid second
    stage on the **singly programmed** view `shiftedOracle`.
  * So after both rewritings the two rounds are `(uniformHashFibers outputs).bind fun
    fibers => (uniformHashFiber wanted).bind fun hash => (hybridStageTwo ... view).map
    Prod.fst` with the *same* samples and the *same* program, and `view` is the only
    difference.
* **The four-point comparison is now family-wide and keyed to the query's own index.**
  `doubledHidden oracle honest steered` is the hidden predicate of a *pair of
  programmings*: a forward query at `index` is hidden when `oracle.permutation index
  input` is the honest or the steered range of that index, an inverse one when its
  argument is the honest range or the erased image `oracle.permutation index label`.
  **`publicAnswer_doubled`** proves the agreement off it for arbitrary `honest`, `steered`,
  `combined` satisfying the three conditions of `programIndices_programIndices`, and
  **`publicAnswer_steeringDouble`** instantiates it at the steering gate. Note that no
  `swapRanges` reparametrisation appears anywhere on this route, exactly as slice 3n
  required. `hybridStageTwo_doubled_agree` then feeds it to `run_idealOracle_agree`: the
  two second stages give the same mass to every `(result, log)` whose log avoids the
  hidden queries.
* **The three obligations of `programIndices_programIndices` are now named lemmas**
  (`steerPrograms_untouched`, `steerPrograms_covered`, `steerPrograms_retargeted`, in
  `Proof/DeferredSteering.lean`), stated for any `shifted` outputs family that agrees with
  `outputs` off the steering gate and takes the value `wanted` at it. They serve both
  `programIndices_steerPrograms` (the `swapRanges` route, unchanged) and the new
  `setSteering` route. The refactor changed no statement.
* **What is left is exactly one probabilistic statement, and its shape is fixed.** Charge,
  on the shifted (reference) side, the event

  > some entry of the final log is `doubledHidden`-hidden, **or** one of the steering's
  > requests is not fresh against the first stage's log.

  Both halves are the census of slice 3n §4.2 -- two points per log entry -- and the
  freshness half is a condition on the *first stage's* log only, because the steering
  programs before the second-stage run starts. The tools are all in place:
  `fiberChunk_mass_le` for the two chunk points, `firstStage_hidden_le` for the label
  point, `erased_touch_le` for the erased-image points (entered through
  `uniform_bind_setOracleAt` + `compatibleLaw_empty` + `pinnedCount_empty`, then
  `compatibleLaw_run` on stage 1, `compatibleLaw_programmed`, then `compatibleLaw_run` on
  stage 2), recombined by `toOuterMeasure_exists_mem_keyed_le` and fed to
  `advantage_bind_le_jointBad`. The honest figure is `4 q / 2 ^ 128` against the reserved
  `8 q / 2 ^ 128`; **no constant moves.**
* **Lean gotchas this slice.**
  * `stageTwoState`'s projections do not reduce during `rw`. A lemma keyed on a bare
    `view` / `table` / `inputMacKey` will not fire against a goal that says
    `(stageTwoState ...).view`; `simp only [stageTwoState_view, stageTwoState_table,
    stageTwoState_inputMacKey, stageTwoState_log, stageTwoState_carrier]` first.
  * `PMF.bind_map` leaves the continuation as `g ∘ f` applied to the bound variable.
    `simp only [Function.comp_apply]` before the next `rw`, or the pattern will not match.
  * `rw [h] at member ⊢` fails outright when `h` has no occurrence in `member`, even
    though it does occur in the goal. Rewrite the two places separately.
  * A tactic `show` cannot use `|>.` pipelining (`... .permutation index |>.symm value`);
    the parser stops at the `=`. Parenthesise.
  * A new leaf file under `Proof/` reaches `Proof/Erased.lean` from neither
    `Proof.VisibleGame` nor `Proof.Assembly`; import it explicitly or
    `programmed_programmed_apply_of_ne` is an unknown identifier.
  * `twoStageGame_congr`'s `joint` hypothesis is stated with the pair written out, so the
    goal after `rw` is `(law.map fun raw => (table raw, hidden raw))`, which is *not*
...
* **Zero `sorry`.** `hybridGame_close_to_idealGame` is
  `workPerAdvantage_of_steeringCharge` (`Proof/Shifted.lean`) applied to
  `le_trans advantage_shiftedReferenceGame_steeredReferenceGame_le steeringCharge_le_steeringStep`.
  The audited statements of `Proof/Privacy.lean` are unchanged and no constant moved:
  `chainPerQuery = 16`, `steeringStep = 8 (q₁ + q₂) / 2 ^ 128 + 6 / 2 ^ 128`, and the hop
  spends `3 q₁ / 2 ^ 128` of it.
* **`Proof/Privacy.lean` had to be split, and the reason is structural rather than
  cosmetic.** `Proof/HybridChain.lean` imported `Proof.Privacy` for `ciphertextBytes`,
  `hybridSimulator`, `hybridGame_eq`, `setBridge`, `mulScalar` and `coreGame`, so importing
  the chain into `Proof/Privacy.lean` -- which the final `exact` needs -- is an import cycle
  (`lake` reports "build cycle detected"). Everything in `Proof/Privacy.lean` upstream of the
  three audited theorems therefore moved **verbatim** into the new `Proof/Hybrid.lean`, and
  `Proof/HybridChain.lean` imports that instead. `Proof/Privacy.lean` now holds exactly
  `realGame_eq_hybridGame`, `hybridGame_close_to_idealGame` and `adaptivePrivacy`, and is the
  last file of the library. Keep it that way: any new game-shape lemma about the hybrid game
  belongs in `Proof/Hybrid.lean`.
* **The reshaping came out simpler than the slice-3q recipe predicted, in one place.** The
  recipe put the sample of the hop at `(fibers, hash, key)` and priced the bad event with two
  `Probability.bind_event_le` peels and a `PMF.toOuterMeasure_map_apply`. That is not needed:
  the charge `3 * budget / 2 ^ 128` is **uniform in both fiber samples**, so those two levels
  are peeled by `advantage_bind_le_pointwise` and the bad event is priced over the **key
  alone**, where `uniform_keyLabel_steeringBlocked_le` applies directly with no joint law and
  no map. The packaged form is `advantage_bind_le_badMiddle`.
* **The ordering constraint of slice 3q is confirmed and is what the code does.** The sample
  order of the hop is `u, tape, carrier, raw, (lazy first stage), fibers, hash, key, oracle,
  body`; `bind_comm_outward` is the one reorder lemma that realises it (four independent
  samples `p₁ p₂ p₃ p₄` drawn as `p₃ p₄ p₂ p₁`, five `PMF.bind_comm`s), and it is used twice:
  once to put the two fiber samples outside the key and the family, and once to put the
  carrier and the raw coordinates outside the oracle and the key at the game level.
* **`advantage_bind_le_pointwise` is now support-restricted**
  (`∀ sample ∈ law.support, …`). That change is forced: the per-transcript bound needs
  `FamilyInjective`, `FamilyCovers` and `familyPinnedCount ≤ q₁`, and
  `lazyFamilyRun_covers` / `lazyFamilyRun_pinnedCount` give those **only on the support**.
  The proof needed one extra case split (`PMF.apply_eq_zero_iff` kills the summand off the
  support) and `PMF.support_nonempty` in place of the `Nonempty Sample` argument.
* **The new lemmas of the slice** (`Proof/SteeringHop.lean`): `bind_comm_outward`,
  `advantage_bind_le_badMiddle`, `toReal_three_budget`,
  **`advantage_bind_familyLaw_shifted_steered_le`** (the per-transcript bound: off the curve
  the two laws are equal, on the curve the charge) and
  **`advantage_shiftedReferenceGame_steeredReferenceGame_le`** (the whole hop as a game).
* **Lean gotchas this slice.**
  * `ENNReal.toReal_ofNat` has to be given **twice** in `toReal_three_budget` -- once for the
    `3` and once for the `2` of `2 ^ 128`; `toReal_two_budget` needs it once only because
    `ENNReal.toReal_pow` leaves the base for the same lemma to consume.
  * Rewriting the stage-two table argument to the key-free form is best done **after**
    `steeredReleasedStageTwo_programAll` / `releasedStageTwo_shifted`, not before: those two
    lemmas need the table in the form `raw.table bridgeKey key` with *their own* key, and
    their conclusions leave `wanted` and `outputs` key-free, so one `rw [tableEq key]` then
    normalises every remaining occurrence at once.
  * When a helper's conclusion is an equality of two fully spelled-out laws, pass the bodies
    as `_` and let unification recover them (`bind_comm_outward _ _ _ _ _`,
    `advantage_bind_le_badMiddle … _ _ …`): the bodies are Miller patterns
    (`body one two three four` at distinct bound variables), so no term has to be written out
    twice.

**Corrections from slice 3q -- THE FAMILY TOOL IS LANDED; WHAT IS LEFT IS THE SAMPLE
ORDER (SUPERSEDED BY SLICE 3R: THE RESHAPING IS DONE).**

* **Route (a) of slice 3p §4.2 was the right choice and it is built: `Proof/Family.lean`
  (~1180 lines, `sorryAx`-free).** The lazy-sampling theorem now tracks **every** fixed-key
  index at once, which is both stronger and *simpler* than the single-index version:
  * `FamilyAssignment = FixedKeyIndex → Assignment`, `FamilyInjective`, `pinFamily` (pin one
    input of one index), `familyLaw assigns` = the product over indices of
    `compatibleLaw (assigns index)`, carried to `PermutationOracle FixedKeyIndex Block`
    through the one-field `oracleEquiv`.
  * **`familyLaw_empty : familyLaw (fun _ _ => none) = PMF.uniformOfFintype
    (PermutationOracle FixedKeyIndex Block)`** -- the eager side of the family theorem is
    *literally* the law the games sample, so `setOracleAt` / `uniform_bind_setOracleAt`
    (slice 3n) are **not needed on this route**. That is a real simplification, not just a
    convenience: the single-index tool forced a resampling hop per index.
  * The product bookkeeping is `Proof/Product.lean` read in both directions:
    **`productPMF_bind_place`** (a coordinate may be drawn first),
    `productPMF_bind_update_unread`, **`productPMF_bind_place_update`**,
    `productPMF_support_mem`, and **`productPMF_bind_pointwise`** (substituting one
    law-preserving transform per coordinate, by `Finset` induction). No new probabilistic
    argument appears anywhere in the file.
  * **`familyLaw_forward` / `familyLaw_inverse`** are `compatibleLaw_forward` /
    `compatibleLaw_inverse` at one coordinate, sandwiched between the two product lemmas.
  * `lazyFamilyAnswer` / `lazyFamilyRun` / `lazyFamily_query_step` / **`familyLaw_run`** /
    **`uniform_run_familyLaw`** mirror their single-index originals, with the `if queryIndex
    = index` branch **gone** (every fixed-key query is tracked, so the handler has two
    interesting cases instead of four).
  * **The accounting is now a pointwise induction, as predicted.**
    `familyPinnedCount assigns = ∑ index, pinnedCount (assigns index)`,
    `familyPinnedCount_pinFamily_le` (one query pins at most one point, at one index) and
    **`lazyFamilyRun_pinnedCount : familyPinnedCount output.2 ≤ familyPinnedCount assigns +
    budget`**. So `Σ_i pinnedCount (assigns i) ≤ q₁` with no hybrid chain and no
    expectation bookkeeping, and the `9 q₁ / 2 ^ 128` of slice 3p §4.2 is gone.
  * `FamilyCovers`, `lazyFamilyAnswer_covers`, **`lazyFamilyRun_covers`** -- the family form
    of the transcript-covers-the-log bridge.
  * **`familyLaw_double`** -- the crown: programming the whole family twice (honestly, then
    at the steering slots) is in law programming it once to the steered ranges, given the
    three transcript conditions at each *touched* index. Its three structural hypotheses are
    exactly `steerPrograms_untouched` / `_covered` / `_retargeted` (slice 3o), so it drops
    straight onto the hop. Untouched indices cost nothing.
* **The charge is machine-checked, in one hop, in concrete game terms.**
  `familyLabelHidden` (the union over indices of the per-index `transcriptHidden`),
  `familyLabelHidden_card ≤ 3 * familyPinnedCount`,
  **`uniform_keyLabel_familyHidden_le`** (mass `≤ 3 * budget / 2 ^ 128`), `steeringBlocked`
  (the bad event, whose negation is `familyLaw_double`'s `good`),
  `good_of_not_steeringBlocked`, `slotChunk` / `slotRange_eq_slotChunk`,
  `mem_familyLabelHidden_of_blocked`, and in `Proof/SteeringHop.lean`
  `selectedChunk` / `steeredChunk` / `selectedPrograms_range` / `steerPrograms_range` /
  `steerPrograms_label` and finally

  ```lean
  theorem uniform_keyLabel_steeringBlocked_le (assigns) (injective) (budget)
      (small : familyPinnedCount assigns ≤ budget) (input outputs rows fibers wanted hash) :
      (PMF.uniformOfFintype InputMacKey).toOuterMeasure
          {key | steeringBlocked assigns (selectedPrograms key input outputs rows fibers)
            (steerPrograms key input wanted hash (rows .x7 0))} ≤ 3 * budget / 2 ^ 128
  ```

  and **`steeringCharge_le_steeringStep : 3 q₁ / 2 ^ 128 ≤ steeringStep`**. So the budget
  question is closed: **no constant moves, there is a factor of two of slack, and the
  `6 / 2 ^ 128` one-time term is entirely unused.**
* **The hop's law equality is machine-checked, at two granularities**
  (`Proof/SteeringHop.lean`, `sorryAx`-free):
  * `steerRequests_fresh_of_covers` -- the freshness half is free, as slice 3p said, but it
    needs `programsOfRequests_of_mem` (a request at a distinct index is the partial
    programming's own request there), which did not exist.
  * **`bind_familyLaw_steeredReleased_eq`** -- the coarse version: with the transcripts good
    for *every* fiber sample, the steered and the shifted second stage are the same law
    (and off the curve, unconditionally).
  * **`bind_familyLaw_programAll_eq_shifted`** -- the version the charge actually consumes,
    with the two fiber samples and the label already fixed. Needs
    `selectedSimulatedStageTwo_programAll` and **`steeredReleasedStageTwo_programAll`**: the
    steering fiber sample exposed **without** the freshness side condition.
* **THE ORDERING CONSTRAINT slice 3p's plumbing plan missed, and it is forced.** The bad
  event mentions the **steered chunk**, which in the `steeringBit = false` branch is a chunk
  of the steering gate's *resampled fiber* `hash`. So the label -- the variable the bad event
  is charged over -- must be sampled **after** both fiber samples, and the fiber samples must
  therefore be exposed as binds *before* the freshness step (which is itself label-dependent).
  Hence:
  1. the hash sample has to be pulled out **unconditionally**
     (`steeredReleasedStageTwo_programAll`, via `steerTo_eq_map`), **not** through
     `selectedSimulatedStageTwo_double`, whose `fresh` hypothesis already needs the label;
  2. the sample order of the hop is
     `u, tape, carrier, raw, (lazy first stage), fibers, hash, key, oracle, body` --
     in particular the **key is sampled last**, after the first stage. That is legitimate
     because the first stage reads the key nowhere: the reference table is key-free
     (`Coordinates.table_key`) and `loggedFirstStage` runs on `witnessTape.inputMacKey`.
  Charging the steered chunk over the *hash* instead (the slice-3l `fiberChunk_mass_le`
  route) does **not** work here: the fiber target is `outputs .x7 0 + (c/s − u)`, which is not
  uniform without deferring the bridge key past the whole game.
* **Half of the reshaping landed too.** `steeredReferenceRound_firstStage_key` /
  `shiftedReferenceRound_firstStage_key` (the first stage's table argument rewritten by
  `Coordinates.table_key` **at that occurrence only** -- a bare `rw` also rewrites the
  stage-2 table, so use `congrArg (fun table => …)`), `bind_key_steeredReferenceRound` /
  `bind_key_shiftedReferenceRound` (the key sample moved past the first stage),
  `bind_loggedFirstStage`, `chargeFirstState` / `setFamily_chargeFirstState`, and
  **`lazy_steeredReferenceRound`** / **`lazy_shiftedReferenceRound`**: both rounds under one
  uniform oracle sample are now

  ```
  (lazyFamilyRun (adversary.chooseInput …) (chargeFirstState rest table) (fun _ _ => none)).bind
    fun output => (familyLaw output.2).bind fun oracle =>
      (uniform InputMacKey).bind fun key => <stage two>
  ```

  i.e. the transcripts are conditioned, the key is last, and only the inner part differs.
* **What is left, in order.**
  1. Per lazy output: one `PMF.bind_comm` puts `key` outside `familyLaw`; then on the curve
     `steeredReleasedStageTwo_programAll` / (`shiftedOutputs = shiftSteering …` +
     `releasedStageTwo_shifted`) expose the two fiber samples and two more `PMF.bind_comm`s
     move `key` *inside* them (both fiber laws are key-free); then
     `advantage_bind_le_bad` over the inner sample `(fibers, hash, key)` with
     `bind_familyLaw_programAll_eq_shifted` as `agreeContinuation` and
     `uniform_keyLabel_steeringBlocked_le` as the mass (two `Probability.bind_event_le`
     peels and one `PMF.toOuterMeasure_map_apply`). **Off the curve the two laws are equal**
     (`steeredReleasedStageTwo_offCurve` + `shiftedOutputs = outputs`), so the case split is
     at this level and no common sample shape has to be forced -- this is simpler than the
     `PMF.bind_const` merge an earlier reading of this plan suggested.
     `familyPinnedCount output.2 ≤ q₁` comes from `lazyFamilyRun_pinnedCount`
     (`familyPinnedCount_empty` + the first stage's budget) and `FamilyCovers` from
     `lazyFamilyRun_covers` (`familyCovers_empty`, and `(chargeFirstState …).log = []`).
     One `ENNReal.toReal` conversion is needed to turn the mass bound into the ℝ-valued
     `advantage` bound.
     One more normalisation is needed first: stage two's **table** argument
     `raw.table bridgeKey key` must be rewritten to `raw.table bridgeKey
     witnessTape.inputMacKey` (`Coordinates.table_key` again), because the bad event's
     `rows` argument has to be key-free for the mass bound to be uniform in the key.
  2. **The averaging tool exists now: `advantage_bind_le_pointwise`** (`Proof/SteeringHop.lean`).
     `advantage_bind_le_of_le` is stated with `[Fintype Sample]`, which the lazy-run output
     type does **not** satisfy (it carries a `List Query` and the adversary's state), so the
     per-transcript bound could not have been averaged with the existing tools. The new
     lemma has no finiteness assumption and its proof avoids real summability entirely: the
     pointwise bound becomes the two `ENNReal` inequalities
     `p true ≤ q true + ENNReal.ofReal bound`, which survive an unconditional `ENNReal`
     tsum. It peels every level of the tower, including `u`, `tape`, `carrier`, `raw`.
  3. `steeringCharge_le_steeringStep`, then
     `exact workPerAdvantage_of_steeringCharge adversary parameter scalar auxiliary charge`.
     The `ENNReal → ℝ` conversion of the mass bound is `toReal_two_budget`'s pattern
     (`Proof/HybridChain.lean`) at the constant `3`.
* **Slice 3p assertions that held up, and one that did not.** Everything in slice 3p §3 and
  §4.2 held up: the per-point constant is `3`, route (a) is the right tool and cost about
  what it predicted, `compatibleLaw_double` is index-free and applies coordinatewise, and
  Lazy.lean's first half was reused verbatim. What did **not** hold up is the *order* of
  slice 3p's plumbing list: "`PMF.bind_comm` to move the two fiber samples inside the
  conditional permutation bind" is backwards -- they must move **outside** it, and outside
  the key sample too, for the reason above.
* **Lean gotchas this slice.**
  * `productPMF` at `Value = Equiv Block Block` needs a `DecidableEq (Equiv Block Block)`
    instance; declare exactly that one (`Classical.propDecidable`) and do **not** open
    `Classical`, which would make `Subtype.fintype` applicable and diverge from Lazy.lean's
    `Fintype.ofFinite` instances.
  * `obtain ⟨index, blocked⟩ := member` on a `key ∈ {key | ∃ index, …}` hypothesis hits
    `maximum recursion depth` at this project's `Set` representation. Pass the membership
    straight to the consumer (`fun key blocked => lemma … blocked`) or raise
    `maxRecDepth`; `set_option maxRecDepth … in` must come **before** the doc comment.
  * `Function.update_self` is what reduces `(Function.update values place value) place`; a
    `rw` chain that needs it cannot be replaced by `rfl`, because `DecidableEq FixedKeyIndex`
    is derived and does not reduce by `whnf`. State the unreduced form in a `have` and
    `simp only [Function.update_self] at` it.
  * `div_add_div_same` does not exist under that name; use `← add_div`. `BitVec.xor_zero`
    does not fire on `(0 : Block)` from `OfNat`; define the chunk directly (`slotChunk`) and
    make the range equation `rfl`.
  * A final `rfl` that has to see through `hybridStageTwo`'s
    `state.inputMacKey.encodeAffine` hits `maximum recursion depth`; finish with
    `simp only [hybridStageTwo, programAll_inputMacKey, stageTwoState_inputMacKey]` instead.
  * `List.mem_singleton_self` takes its element **explicitly**.
  * `mem_familyLabelHidden_of_blocked` must take the "all labels are the steering label"
    hypothesis on the **steered** programs, not the honest ones: `selectedPrograms` programs
    every gate at *its own* label, so the honest-side version is false.

**P10 is DONE (slice 3r).** Nothing is left: `hybridGame_close_to_idealGame` is proved and
the tree has zero `sorry`. The texts below are kept for the routes they record, not for
their to-do lists.
