/-
This file builds the **hybrid game** -- the real game written in the simulator's
shape -- and the shared core both games project onto.

Its first stage samples the same tape and carrier as the simulator but garbles
the curve table for the bridge key `carrier / scalar`, so the honest labels
already release `carrier / scalar` and no programming is needed. The bridge-key
bijection (`uniform_bind_setBridge`, `mulScalar`) is what turns the real game's
own uniform bridge key into that sample, and the core game
(`bind_run_project`) is the part of both games that never looks at the
difference.

The obligation itself is stated in `Proof/Privacy.lean`, which consumes the
whole chain; everything here is upstream of it, so the chain's files import this
